...
describe("wt_review", function()
	...
	describe("resolve_base_ref()", function()
		...
		it("returns the merge-base commit hash (not a branch name)", function()
			...
			assert.matches("^[a-f0-9]+$", base_ref, "resolve_base_ref should return a commit hash, not a branch name")
		end)
	end)
	...
	describe("get_changed_files(base_ref)", function()
		it("exists as a function on the module", function()
			...
			assert.is.Function(wt_review.get_changed_files, "get_changed_files should be a function")
		end)
		...
		it("returns a sorted list of changed files with +N/-M stats", function()
			...
			for i = 2, #files do
				assert.is_true(
					...
					string.format("files should be sorted by path: %s < %s", files[i].path, files[i - 1].path)
				)
			end
		end)
		...
		it("includes untracked files in the result", function()
			...
			-- Create a temporary untracked file so the test is self-contained
			local tmp_file = "tests/_untracked_test_tmp.lua"
			local fd = io.open(tmp_file, "w")
			if fd then
				fd:write("-- temporary untracked file for wt_review test\n")
				fd:close()
			end
			...
			-- Untracked files should appear
			local has_untracked = false
			for _, f in ipairs(files) do
				if f.path == tmp_file then
					has_untracked = true
					break
				end
			end
			assert.is_true(has_untracked, "should include untracked test file: " .. tmp_file)
		end)
	end)
	...
	describe("open_sidebar(changed_files)", function()
		it("exists as a function on the module", function()
			local wt_review = require("wt_review")
			assert.is.Function(wt_review.open_sidebar, "open_sidebar should be a function")
		end)
		...
		it("creates a new buffer with 'wtreview' filetype", function()
			...
			assert.is_true(buf_count_after > buf_count_before, "open_sidebar should create a new buffer")
		...
		it("sorts files by path in the buffer", function()
			...
			for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
				if vim.api.nvim_buf_is_valid(bufnr) and vim.bo[bufnr].filetype == "wtreview" then
					...
					assert.is_true(alpha_pos < zeta_pos, "alpha/file.txt should appear before zeta/file.txt")
	...
	describe("open_file_diff(file_path, base_ref)", function()
		it("exists as a function on the module", function()
			local wt_review = require("wt_review")
			assert.is.Function(wt_review.open_file_diff, "open_file_diff should be a function")
		end)
		...
		it("opens the working tree file in the current window", function()
			...
			assert.is_true(vim.api.nvim_win_is_valid(cur_win), "current window should be valid")
			-- The buffer should be a normal file buffer (not nofile)
			assert.is_not.equals("nofile", vim.bo[cur_buf].buftype, "working tree buffer should not be nofile")
			-- Should be modifiable
			assert.is_true(vim.bo[cur_buf].modifiable, "working tree buffer should be modifiable")
		end)
		...
		it("opens the base version in a vertical split with content from git show", function()
			...
			local ok, expected_lines = pcall(vim.fn.systemlist, "git show " .. base_ref .. ":" .. test_file)
			assert.is_true(ok and vim.v.shell_error == 0, "git show should work on " .. test_file)
			...
			assert.is_true(#wins_after >= #wins_before + 1, "open_file_diff should create a vertical split")
		...
		it("sets diff mode on both windows", function()
			...
			assert.is_true(diff_wins >= 2, "at least 2 windows should have diff mode enabled")
		end)
	end)
	...
	describe("review comment integration", function()
				...
				if vim.api.nvim_buf_is_valid(b) then
					...
					elseif vim.bo[b].buftype == "" then
						...
						for _, w in ipairs(vim.api.nvim_list_wins()) do
							if vim.api.nvim_win_is_valid(w) and vim.api.nvim_win_get_buf(w) == b and vim.wo[w].diff then
								work_bufnr = b
								break
							end
						end
		...
		describe("map_diff_line()", function()
			it("exports map_diff_line function on the module", function()
				local wt_review = require("wt_review")
				assert.is.Function(wt_review.map_diff_line, "map_diff_line should be a function")
			end)
			...
			it("maps line 1 from wtdiff to working tree correctly", function()
				...
				assert.is_not_nil(mapped, "line 1 in base should map to a line in working tree")
				assert.equals(1, mapped, "first line of base version should map to first line of working tree")
			end)
			...
			it("returns nil for lines beyond buffer bounds", function()
				...
				assert.is_nil(mapped, "line beyond buffer bounds should return nil")
			end)
		end)
		...
		describe("add_comment()", function()
			it("exports add_comment function on the module", function()
				local wt_review = require("wt_review")
				assert.is.Function(wt_review.add_comment, "add_comment should be a function")
			end)
			...
			it("calls review.add() with correct line numbers from working tree buffer", function()
				...
				assert.equals(5, captured_start, "add_comment should pass correct line_start from normal buffer")
				assert.equals(5, captured_end, "add_comment should pass correct line_end from normal buffer")
	...
	describe("open(ref) — full review orchestration", function()
		it("exports open function on the module", function()
			local wt_review = require("wt_review")
			assert.is.Function(wt_review.open, "open should be a function on wt_review")
		end)
		...
		it("open() creates sidebar + opens first file diff when called with a ref", function()
			...
			assert.is_true(buf_count_after > buf_count_before, "open() should create at least one new buffer")

			-- Should have created new windows (sidebar split + optionally diff split)
			assert.is_true(win_count_after > win_count_before, "open() should create new windows")
	...
	describe("wpi TUI integration", function()
		it("wpi TUI source references wt_review instead of git_diff_review", function()
			...
			assert.is_not_nil(wt_pos, "wpi TUI source should contain 'wt_review'")

			-- The TUI should NOT reference the old git_diff_review module
			local old_pos = content:find("git_diff_review", 1, true)
			assert.is_nil(old_pos, "wpi TUI source should NOT contain 'git_diff_review'")
		end)
		...
		it("wpi TUI passes WPI_BASE_REF environment variable", function()
			...
			assert.is_not_nil(content:find("WPI_BASE_REF"), "wpi TUI should still set WPI_BASE_REF env var")
		end)
	end)
end)
